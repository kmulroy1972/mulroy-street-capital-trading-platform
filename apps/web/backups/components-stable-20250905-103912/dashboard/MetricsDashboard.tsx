...
export default function MetricsDashboard({ editMode = false }: MetricsDashboardProps) {
  ...
  const fillRate = metrics?.orders ? ((metrics.orders.filled ?? 0) / (metrics.orders.placed || 1) * 100) : 0;
  const rejectionRate = metrics?.orders ? ((metrics.orders.rejected ?? 0) / (metrics.orders.placed || 1) * 100) : 0;
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div>
            ...
            <div className="text-red-400 font-medium">
              {((metrics?.risk?.max_drawdown || 0) * 100)?.toFixed(1) || "0.0"}%
            </div>
          </div>
        </div>
